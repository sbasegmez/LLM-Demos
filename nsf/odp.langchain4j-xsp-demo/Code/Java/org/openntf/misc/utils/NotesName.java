package org.openntf.misc.utils;
 ...
 * 
 *        This initial version is a very basic implementation for Notes Names in
 *        Java, of course without Session-binded Name class.
 * 
 *        It parses very basic names. Supports common name, OUs, Organization
 *        and Country. Use with caution regarding the following issues:
 * 
 *        - It does not check some of the naming standards like allowing more
 *        than 4 OUs. - Some illegal names might return unexpected results, like
 *        "SomeName//us" or "CN=Some Name/O=Company=some" - Null or empty name
 *        will result an empty name with no components.
 * 
 * 
 * @author sbasegmez
 * @version version 1.2 - 2022/08/31 - Added Java 8 streams to static functions.
 *          - Removed in-place update functions.
 * 
 *          version 1.1 - 2014/08/17 - Added static methods toCommon,
 *          toAbbreviated, toCanonical - Added static methods setNamesCommon,
 *          setNamesAbbreviated, setNamesCanonical - Added getCommon() alias for
 *          getCommonName()
 * 
...
public class NotesName implements Serializable {
	...
	protected static boolean isCanonical(String name) {
		// The rule is simple. Every items in the '/' seperated part should have '='
		// sign.
	...
	protected static void nameTest(String name) {
		try {
			System.out.println(
					"Given Name : '" + name + "' (" + (isCanonical(name) ? "Canonical" : "Abbreviated") + ")");
